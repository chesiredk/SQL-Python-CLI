...
class Hobby():
    ...
    table = "hobby"
    print("Table Name: HOBBY with hobbyID and HobbyName")

    def create(dbms, myCursor):
        myCursor.execute("SELECT HobbyID FROM db_class.hobby;")
        for i in myCursor:
            print(i)
        newhobbyName = input("What hobby would you like to add? ")
        newhobbyID = input("New HobbyID? That doesn't resemble none of the above")
        command = "INSERT INTO db_class.hobby(HobbyID,HobbyName) VALUES (" + newhobbyID + ", " + newhobbyName + ');'
        print(command)
    ...
    def read(myCursor, table):
        rOption = input("Do you want to read all entries from the " + table + " table? ")
        if rOption == "yes":
        ...
        if rOption == "no":
            ...
            command += columns
            additional_cmd = " FROM db_class." + table + " WHERE HobbyID = "
    ...
    def delete(dbms, myCursor, table):
        ...
        if del_Option == "yes":
            ...
            command = "DELETE FROM db_class." + table + " WHERE HobbyID= "
            conditional = input("Enter HobbyID's #")
